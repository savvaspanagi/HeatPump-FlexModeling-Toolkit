...
# ---- Main function ----
def build_greybox_model(train_df, model_type, deltaT, solver_name="ipopt", Tee=False, initialization=None, bounds=None, show_plot=False):
    model = ConcreteModel()
    N = len(train_df)
    ...
    param_defaults, param_bounds = get_model_parameters(model_type, initialization, bounds)

    # Define variables
    define_variables(model, model_type, param_defaults, param_bounds)

    # Define states
    define_states(model, model_type)
    ...
    # Dynamics
    define_dynamics(model, model_type, deltaT, Q_h, Q_irrad, T_out_meas, T_in_meas)
    ...
    # Extract parameters
    param_values = extract_parameters(model, model_type,solve_time)

    if show_plot:
        plot_temperature_comparison(model, train_df, T_in_meas)

    return model, param_values

# --- Supporting functions ---

def get_model_parameters(model_type, initialization=None, bounds=None):
    param_defaults = {}
    param_bounds = {}
...
def define_variables(model, model_type, param_defaults, param_bounds):
...
def define_states(model, model_type):
    # Define T_in with bounds [-30, 50]
    model.T_in = Var(model.T, bounds=(-30, 65))
...
def define_dynamics(model, model_type, deltaT, Q_h, Q_irrad, T_out_meas, T_in_meas):
    N = len(Q_h)
...
def extract_parameters(model, model_type,solve_time):
...
def plot_temperature_comparison(model, train_df, T_in_meas):
    N = len(train_df)
