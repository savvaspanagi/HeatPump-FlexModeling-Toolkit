...
# def simulate_and_evaluate_q_models(val_df, param_dict,initializations, model_type="1R1C", deltaT=1800, show_plot=True):
#     N = len(val_df)
...
def simulate_Qh(df, param_dict, initializations=None, model_type="1R1C", deltaT=1800, show_plot=True):
    N = len(df)
    ...
    for t in range(N - 1):
        ...
        if model_type == "1R1C":
            Q_h[t] = C_in * dT_in - (T_out[t] - T_in[t]) / R_in_a - Ain * Q_irrad[t]
            print("1R1C")
            
        elif model_type == "2R2C_A":
            print("2R2C_A")
            Q_h[t] = (C_in / fh) * dT_in - (T_e[t] - T_in[t]) / (R_in_e * fh) 
            - Ain * Q_irrad[t] / fh
            ...
            Q_h[t] = ((C_in / fh) * dT_in - (T_e[t] - T_in[t]) / (R_in_e * fh) - 
            (T_out[t] - T_in[t]) / (R_in_a * fh) - Ain * Q_irrad[t] / fh)
            T_e[t + 1] = T_e[t] + deltaT * (
                (T_out[t] - T_e[t]) / (R_e_a * C_e) +
                (T_in[t] - T_e[t]) / (R_in_e * C_e) +
                Ae * Q_irrad[t] / C_e +
            ...
            Q_h[t] = (C_in / fh_in) * dT_in - (T_e[t] - T_in[t]) / (R_in_e * fh_in) - (T_out[t] - T_in[t]) / (R_in_a * fh_in) - (T_int[t] - T_in[t]) / (R_int_in * fh_in) - Ain * Q_irrad[t] / fh_in 
    ...
    if show_plot:
        ...
        plt.plot(df.index, df["PHI_H"], label="Measured Phi_h", linestyle="--", alpha=0.5)
        plt.xlabel("Time")
